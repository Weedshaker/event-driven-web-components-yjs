    ...
    new Promise(resolve => this.dispatchEvent(new CustomEvent('yjs-get-room', {
      ...
      if (!room.done) {
        await new Promise(resolve => this.dispatchEvent(new CustomEvent('yjs-set-room', {
          detail: {
            room: self.prompt('room-name', `random-room-${new Date().getUTCMilliseconds()}`) || 'weedshakers-event-driven-web-components-chat',
            resolve
          },
          bubbles: true,
          cancelable: true,
      ...
      this.dispatchEvent(new CustomEvent('yjs-set-local-state-field', {
        ...
        detail: {
          value: {
            nickname: (nickname = self.localStorage.getItem(await room + '-nickname') || self.prompt('nickname', `${nickname}-${new Date().getUTCMilliseconds()}`) || nickname)
          }
        },
        bubbles: true,
        cancelable: true,
